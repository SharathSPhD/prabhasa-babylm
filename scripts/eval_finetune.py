...
def main() -> None:
    ...
    ap.add_argument("--tasks", nargs="+", default=list(GLUE_TASKS),
                    choices=list(GLUE_TASKS), help="Subset of GLUE tasks to run.")
    ...
    ap.add_argument("--glue-dir", default="evaluation_data/full_eval/glue_filtered",
    ...
    for task in args.tasks:
        ...
        print(f"[finetune] {task} (labels={cfg['num_labels']}, epochs={epochs}, batch={batch})",
              flush=True)
        cmd = [
            PY, "-m", "evaluation_pipeline.finetune.run",
            "--model_name_or_path", str(hf_dir),
            "--train_data", f"{glue_dir}/{task}.train.jsonl",
            "--valid_data", f"{glue_dir}/{task}.valid.jsonl",
            "--predict_data", f"{glue_dir}/{task}.valid.jsonl",
            "--task", task, "--num_labels", str(cfg["num_labels"]),
            "--batch_size", str(batch), "--learning_rate", str(args.lr),
            "--num_epochs", str(epochs), "--sequence_length", str(args.seq_len),
            "--results_dir", args.results_dir, "--seed", str(args.seed),
            "--metrics", *metrics, "--metric_for_valid", cfg["valid_metric"], "--verbose",
        ]
        subprocess.run(cmd, cwd=PIPELINE, check=True)
        res_path = PIPELINE / args.results_dir / args.name / "main" / "finetune" / task / "results.txt"
    ...
    out_obj = {"name": args.name, "tasks": summary, "glue_average": glue_avg,
