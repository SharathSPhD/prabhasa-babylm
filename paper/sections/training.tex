...
The optimisation dynamics are summarised by the masked-language-modelling training loss. Figure~\ref{fig:trainloss} plots the training loss of the 100M Strict run against optimisation steps; the loss trends downward under the Muon optimiser and the cosine masking schedule, with the final value reported in Table~\ref{tab:training_config_full}. The 10M run is omitted from the figure because the two recorded seed logs are identical and would misrepresent seed variability.

\begin{figure}[t]\centering\includegraphics[width=0.7\linewidth]{figures/fig_trainloss.pdf}
\caption{Masked-language-modelling training loss of the 100M Strict run against optimisation steps, showing an overall downward trend with oscillations characteristic of small-batch optimisation.}
...
\begin{table}[t]
\centering
\caption{Training configuration: vocabulary 20K, sequence length 192, batch size 256, cosine mask schedule 0.40 to 0.15, Muon, RoPE. Production and mechanism-ablation arms shown.}
