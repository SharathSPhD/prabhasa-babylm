...
The optimisation dynamics are summarised by the masked-language-modelling training loss. Figure~\ref{fig:trainloss} plots the training loss against optimisation steps for the production runs; both runs trend downward under the Muon optimiser and the cosine masking schedule, with the final loss values reported in Table~\ref{tab:training_config_full}.

\begin{figure}[t]\centering\includegraphics[width=0.85\linewidth]{figures/fig_trainloss.pdf}
\caption{Masked-language-modelling training loss against optimisation steps for the Strict (100M) and Strict-Small (10M) production runs. The 10M run (left) converges smoothly, while the 100M run (right) exhibits oscillations characteristic of small-batch stochastic optimisation, with an overall downward trend and no divergence.}
...
\begin{equation}
p_{\text{mask}}(t) = p_{\text{base}} + \delta_{r(t)},
\end{equation}
...
\begin{equation}
\mathcal{L}_{\text{aux}} = -\sum_{t} \sum_{r=1}^{10} y_r(t) \log \hat{p}_r(t),
\end{equation}
...
\midrule
prabhasa-b\_ss-0.1 (Submission) & 10M & Hybrid & 3 & N-hot + RoPE & 1e-3 & 64.09 ± 0.26 \\
