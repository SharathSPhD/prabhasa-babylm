...
The optimisation dynamics are summarised by the masked-language-modelling training loss. Figure~\ref{fig:trainloss} plots the training loss against optimisation steps for the production runs, showing smooth convergence under the Muon optimiser and the cosine masking schedule, with the final loss values reported in the configuration table.

\begin{figure}[t]\centering\includegraphics[width=0.85\linewidth]{figures/fig_trainloss.pdf}
\caption{Masked-language-modelling training loss against optimisation steps for the Strict (100M) and Strict-Small (10M) production runs. Loss decreases smoothly under the Muon optimiser and the cosine masking schedule, without instability or divergence.}
\label{fig:trainloss}\end{figure}
