...
\noindent\textbf{Keywords:} data-efficient pretraining; BabyLM; P\=a\d{n}inian grammar; k\=araka roles; masked language modelling; morphology-aware masking.
...
On the internal harness, the Strict submission reaches 73.06 BLiMP, within 1.5 points of the 74.53 baseline, and the Strict-Small submission reaches a 3-seed mean of 64.09 ($\pm$0.26) against the 65.08 baseline (Table~\ref{tab:headline}). The Strict-Small submission keeps the hybrid objective because F1 shows the pure-MLM advantage only emerges at 100M. Fine-tuned on seven GLUE tasks \cite{Wang2019GLUE}, the official-pipeline macro-averages are 63.28 (Strict) and 63.09 (Strict-Small), strongest on MRPC and WSC and weakest on the multi-class task MNLI (\ref{app:repro}, Table~\ref{tab:official_glue})---the expected profile for these data budgets.
...
Epochs & 10 \\
Mask-rate schedule & cosine $0.40\!\to\!0.15$ \\
\midrule
Objective (Strict) & pure MLM \\
Objective (Strict-Small) & hybrid MLM\,+\,CLM \\
CLM head weight (hybrid) & 0.5 \\
N-hot role dimension & 10 \\
Auxiliary weight $\lambda$ & 1.0 (when enabled) \\
Frequency weight $\alpha$ (F2) & 0.5 / 0 (matched) \\
\midrule
Seeds & 3 (10M), 1 (100M arms), 5 (F3) \\
Hardware & 1$\times$128\,GB unified-mem.\ GPU \\
\bottomrule
\end{tabular}
\end{table}
