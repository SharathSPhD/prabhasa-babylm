...
about what the approach does not claim. At proxy scale on a compositional
argument-role venue the P\=a\d{n}inian prior confers no measurable advantage over
a surface-matched control, a clean controlled negative that we interpret as venue
saturation rather than universal absence of benefit; we then re-test the question
from scratch under the BabyLM Strict-Small protocol with four token-matched dose
arms evaluated on the official pseudo-log-likelihood suite, and we separate the
budget-controlled scientific comparison from a leaderboard-submission track that
layers data-efficiency mechanisms studied independently.
\end{abstract}
...
per-source SHA-256 in the corpus manifest.}
\label{tab:arms}
\begin{tabular}{llrrr}
\toprule
...
D & Paribh\=a\d{s}\=a & $20{,}581$ & $67{,}218$ & $1{,}498{,}163$ \\
\bottomrule
\end{tabular}
\end{table}
